...
\subsection{A Note on the Cost of AI Project Suggestions}\label{section.applications.cost}
The projects we outline in this section would incur several different types of cost.  Some of the computation would take place on servers in the Miller Center's private AWS cloud.  However, the bulk of the proposed work would take the form of calls to public API's (e.g. from OpenAI, AWS, Google).  API costs come in two varieties:
\begin{enumerate}
\item One-time upfront costs for model training
\item Ongoing cost for workload requests.
\end{enumerate}
At this time, AI companies charge for API usage on a per-token basis.  The is, the more word tokens we ask the model to read or create, the higher our cost.  These costs vary from service to service.  But to give a sense of their magnitude we offer these data points:
\begin{itemize}
\item To fine-tune GPT-4 for OHP interview summarization would cost approximately \$30 as of this writing.
\item To use OpenAI's Whisper audio model to transcribe the full corpus of PRP recordings would cost about \$1,750.
\end{itemize}
The actual cost of training models may end up somewhat different from these numbers, but they give a good indication of the order of cost magnitude the Miller Center would face to train AI models.

The ongoing deployment and hosting costs are harder to predict, partly because we do not yet know the details of what we would deploy, and partly because we do not know the amount of traffic we would have to support.  It would, for instance, cost much more to serve a Miller Center LLM for the general public than for local scholars only.  

Another ongoing expense would be the cost of housing any locally-served models.  It may be the case that public APIs give enough flexibility that we would never need to train and send traffic to a locally owned model.  However, depending on what our precise needs end up being, we may be better served by  training a model from scratch or fine tuning a model that we own and maintain\footnote{The decision to host our own model instead of using an API-based model in the cloud will hinge on the exact nature of the system and training we need to implement.  Using a cloud-based model is likely to be easier in general.  But in certain cases, public training APIs do not support local needs, or they can end up being more expensive than serving a local model.}.  A common industry rule of thumb is that if our traffic generates more than about 1,000 model invocations per day, hosting our model becomes cost effective \cite{vivek:2023, javaness:2023}.  With traffic below about 1,000 calls per day, relying on public APIs is usually cheaper.  

In general, the cost of providing access to LLMs is an ongoing proposition.  This expense scales linearly with the amount of traffic the models respond to (both input and output)
...
\item Miller Center staff would develop software using the OpenAI ChatGPT and LangChain API's.  This software handles incoming user queries, searches over the vector database, and finally, translation of retrieved documents into conversational English.
\end{enumerate}
...
The Miller Center's Presidential Oral History Project (POHP) generates rich, long-form interviews with former White House personnel and other presidential confidants.  To maximize the value of the corpus of these interviews, two summary bits of metadata are desirable:
\begin{enumerate}
...
def main(pdf_path):
    ...
    for page_number, text in extract_text_from_pdf(pdf_path):
        page_number += 1
        prompt = "List all person, country , and government agency names contained in the following text: " + text
...
if __name__ == "__main__":
    ...
    main(pdf_path)
\end{lstlisting}
\caption{A Sample Python Program for Automatically Extracting Indexing Terms from an Interview.}
\label{figure.python}
\end{figure}
...
There is, however, one point of difficulty that we must solve before deploying an automatic pr\'{e}cis creation tool.  The problem has to do with the ``context window" of LLMs.  The context window is simply the maximum allowable length of an LLM's prompts.  Each model's context window is different, and for the most part, larger models have larger context windows.  GPT-4's context window is 8,000 tokens (approximately 8,000 words).  For comparison, the Elliott Abrams interview shown in Figure \ref{figure.python} is 95,693 tokens, as per OpenAI's counting method.  Thus we must devise a way to extract a single, summative pr\'{e}cis from GPT-4 (or any fine-tuned model derived from it), when we can only show $\frac{1}{11}^{th}$ of the interview to the model at a time.

How to overcome this context window limitation is an open problem, a viable research question in its own right.  However, our initial experiments suggest that an iterative algorithm can be used to solve this issue.  The algorithm runs as follows:
\begin{enumerate}
\item Begin with input interview $D$.
\item Split $D$ into $n$ sub-documents $D_1, D_2, \ldots, D_n$ such that  $length(D_i < 8000)$.
...
\end{enumerate}
Testing and troubleshooting this algorithm will require some work, but initial experiments suggest that it is feasible and practical.  
